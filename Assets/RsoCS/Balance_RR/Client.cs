namespace rso
{
    ...
    namespace balance_rr
    {
        ...
        public class CClient
        {
            ...
            public bool Connect(TPeerCnt PeerNum_, string DataPath_, CNamePort AllocatorNamePort_)
            {
                try
                {
                    CNamePort AllocatorNamePort;
                    var DataFile = DataPath_ + "Allocator_" + PeerNum_.ToString() + ".dat";
                    var itServer = _Servers.AddAt((Int32)PeerNum_, new _SServer(DataFile));
                    if (itServer.Data.NamePort.Data)
                        AllocatorNamePort = itServer.Data.NamePort.Data;
                    else
                        AllocatorNamePort = AllocatorNamePort_;

                    if (_Net.Connect(AllocatorNamePort, PeerNum_) == null)
                    {
